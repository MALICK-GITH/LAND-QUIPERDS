...
-- Bootstrap schema for Supabase.
-- Run this in the Supabase SQL editor on an empty database.
...
-- Auth helpers must exist before RLS policies use them.
...
BEGIN
  INSERT INTO public.transactions (
    ...
    description, related_duel, related_deposit, related_withdrawal
  )
  SELECT
    p_user, w.id, p_type, p_amount, p_before, p_after,
    p_desc, p_duel, p_deposit, p_withdrawal
...
BEGIN
  ...
  IF p_challenged = v_me THEN RAISE EXCEPTION 'You cannot challenge yourself'; END IF;
  IF p_amount <= 0 THEN RAISE EXCEPTION 'Invalid amount'; END IF;
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id = v_me AND (is_banned OR status <> 'active')) THEN
    RAISE EXCEPTION 'Your account cannot create challenges';
  END IF;
  ...
  PERFORM public._notify(
    ...
    'You have received a challenge of ' || p_amount || ' FCFA.',
    '/defis'
  );
...
BEGIN
  ...
  IF v_me NOT IN (c.challenger_id, c.challenged_id) THEN RAISE EXCEPTION 'Access denied'; END IF;
  IF c.status NOT IN ('pending', 'counter_offer') THEN
  ...
  IF p_action = 'cancel' THEN
    IF v_me <> c.challenger_id THEN RAISE EXCEPTION 'Only the sender can cancel'; END IF;
    UPDATE public.challenges SET status = 'cancelled' WHERE id = c.id;
    RETURN NULL;
  END IF;

  IF v_me <> v_responder THEN
  ...
  IF p_action = 'decline' THEN
    ...
    PERFORM public._notify(
      CASE WHEN v_me = c.challenger_id THEN c.challenged_id ELSE c.challenger_id END,
      'challenge_declined',
      'Challenge declined',
  ...
  IF p_action = 'counter' THEN
    ...
    UPDATE public.challenges
      SET status = 'counter_offer',
          accepted_amount = p_counter_amount
      WHERE id = c.id;
    PERFORM public._notify(
      c.challenger_id,
      'challenge_counter',
      'Counter offer received',
      'New amount: ' || p_counter_amount || ' FCFA.',
      '/defis'
    );
    RETURN NULL;
  END IF;
  ...
  IF v_b1 < v_amount THEN RAISE EXCEPTION 'Insufficient balance on challenger side'; END IF;
  IF v_b2 < v_amount THEN RAISE EXCEPTION 'Insufficient balance on opponent side'; END IF;

  v_rate := public.get_commission_rate(v_amount);

  INSERT INTO public.duels (
    player1_id, player2_id, amount, commission_rate, commission_amount, challenge_id, status
  )
  VALUES (
    c.challenger_id,
    c.challenged_id,
  ...
  UPDATE public.wallets
    SET balance_available = balance_available - v_amount,
        balance_locked = balance_locked + v_amount
  ...
  PERFORM public._record_tx(c.challenger_id, 'stake_locked', v_amount, v_b1, v_b1 - v_amount, 'Stake locked for duel', v_duel);
  PERFORM public._record_tx(c.challenged_id, 'stake_locked', v_amount, v_b2, v_b2 - v_amount, 'Stake locked for duel', v_duel);
  ...
  PERFORM public._notify(c.challenger_id, 'duel_started', 'Duel started', 'Your duel has started.', '/duels/' || v_duel);
  PERFORM public._notify(c.challenged_id, 'duel_started', 'Duel started', 'Your duel has started.', '/duels/' || v_duel);

  RETURN v_duel;
END;
$$;

CREATE OR REPLACE FUNCTION public._settle_duel(
  p_duel uuid,
  p_outcome text,
...
BEGIN
  ...
  IF d.status IN ('finished', 'cancelled') THEN
    RAISE EXCEPTION 'Duel already settled';
  END IF;
  ...
  IF p_outcome = 'winner' THEN
    IF p_winner IS NULL OR p_winner NOT IN (d.player1_id, d.player2_id) THEN
    ...
    UPDATE public.wallets
      SET balance_locked = balance_locked - d.amount,
    ...
    UPDATE public.wallets
      SET balance_locked = balance_locked - d.amount,
          total_lost = total_lost + d.amount
      WHERE user_id = v_loser;

    PERFORM public._record_tx(p_winner, 'win', v_payout, v_bw, v_bw + v_payout, 'Duel win', d.id);
    PERFORM public._record_tx(p_winner, 'commission', v_com, NULL, NULL, 'Platform commission', d.id);
    PERFORM public._record_tx(v_loser, 'loss', d.amount, v_bl, v_bl, 'Lost stake', d.id);

    UPDATE public.profiles
      SET wins = wins + 1,
          current_streak = current_streak + 1,
          total_earnings = total_earnings + (v_payout - d.amount)
      WHERE id = p_winner;
    ...
    UPDATE public.duels
      SET status = 'finished',
          ...
          loser_id = v_loser,
          is_draw = false,
          commission_amount = v_com,
          finished_at = now(),
          admin_note = COALESCE(p_note, admin_note)
      WHERE id = d.id;

    PERFORM public._notify(p_winner, 'duel_won', 'Duel won', 'You won ' || v_payout || ' FCFA.', '/duels/' || d.id);
    PERFORM public._notify(v_loser, 'duel_lost', 'Duel lost', 'You lost your stake of ' || d.amount || ' FCFA.', '/duels/' || d.id);
  ...
  ELSIF p_outcome = 'draw' THEN
    ...
    UPDATE public.wallets
      SET balance_locked = balance_locked - d.amount,
          balance_available = balance_available + v_refund
      WHERE user_id IN (d.player1_id, d.player2_id);

    PERFORM public._record_tx(d.player1_id, 'stake_refunded', v_refund, v_b1, v_b1 + v_refund, 'Partial refund after draw', d.id);
    PERFORM public._record_tx(d.player2_id, 'stake_refunded', v_refund, v_b2, v_b2 + v_refund, 'Partial refund after draw', d.id);
    PERFORM public._record_tx(d.player1_id, 'commission', ROUND(v_com / 2, 2), NULL, NULL, 'Platform commission (draw)', d.id);
    PERFORM public._record_tx(d.player2_id, 'commission', ROUND(v_com / 2, 2), NULL, NULL, 'Platform commission (draw)', d.id);

    UPDATE public.profiles
      SET draws = draws + 1,
          current_streak = 0
      WHERE id IN (d.player1_id, d.player2_id);
    ...
    PERFORM public._notify(d.player1_id, 'duel_draw', 'Draw', 'Refund of ' || v_refund || ' FCFA.', '/duels/' || d.id);
    PERFORM public._notify(d.player2_id, 'duel_draw', 'Draw', 'Refund of ' || v_refund || ' FCFA.', '/duels/' || d.id);
  ...
  ELSIF p_outcome = 'cancel' THEN
    ...
    UPDATE public.wallets
      SET balance_locked = balance_locked - d.amount,
          balance_available = balance_available + d.amount
      WHERE user_id IN (d.player1_id, d.player2_id);

    PERFORM public._record_tx(d.player1_id, 'stake_refunded', d.amount, v_b1, v_b1 + d.amount, 'Duel cancelled: full refund', d.id);
    PERFORM public._record_tx(d.player2_id, 'stake_refunded', d.amount, v_b2, v_b2 + d.amount, 'Duel cancelled: full refund', d.id);
    ...
    PERFORM public._notify(d.player1_id, 'duel_cancelled', 'Duel cancelled', 'Your stake was refunded.', '/duels/' || d.id);
    PERFORM public._notify(d.player2_id, 'duel_cancelled', 'Duel cancelled', 'Your stake was refunded.', '/duels/' || d.id);
  ...
  ELSIF p_outcome = 'cancel_no_refund' THEN
    ...
    PERFORM public._record_tx(d.player1_id, 'loss', d.amount, NULL, NULL, 'Duel cancelled without refund', d.id);
    PERFORM public._record_tx(d.player2_id, 'loss', d.amount, NULL, NULL, 'Duel cancelled without refund', d.id);

    UPDATE public.duels
      SET status = 'cancelled',
          commission_amount = v_pot,
          finished_at = now(),
          admin_note = COALESCE(p_note, admin_note)
      WHERE id = d.id;

    PERFORM public._notify(d.player1_id, 'duel_cancelled', 'Duel cancelled', 'Cancellation without refund.', '/duels/' || d.id);
    PERFORM public._notify(d.player2_id, 'duel_cancelled', 'Duel cancelled', 'Cancellation without refund.', '/duels/' || d.id);
  ELSE
...
CREATE OR REPLACE FUNCTION public.submit_duel_vote(
  p_duel uuid,
  p_vote public.duel_vote
)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_me NOT IN (d.player1_id, d.player2_id) THEN RAISE EXCEPTION 'Access denied'; END IF;
  IF d.status NOT IN ('active', 'waiting_votes') THEN
    RAISE EXCEPTION 'This duel no longer accepts votes';
  END IF;

  IF v_me = d.player1_id THEN
    IF d.player1_vote IS NOT NULL THEN RAISE EXCEPTION 'You already voted'; END IF;
    UPDATE public.duels
      SET player1_vote = p_vote,
          player1_voted_at = now(),
          status = 'waiting_votes'
      WHERE id = d.id;
  ELSE
    IF d.player2_vote IS NOT NULL THEN RAISE EXCEPTION 'You already voted'; END IF;
    UPDATE public.duels
      SET player2_vote = p_vote,
          player2_voted_at = now(),
          status = 'waiting_votes'
      WHERE id = d.id;
  END IF;
  ...
  IF v1 = 'win' AND v2 = 'lose' THEN
    PERFORM public._settle_duel(d.id, 'winner', d.player1_id);
    RETURN 'player1';
  ELSIF v1 = 'lose' AND v2 = 'win' THEN
    PERFORM public._settle_duel(d.id, 'winner', d.player2_id);
    RETURN 'player2';
  ELSIF v1 = 'draw' AND v2 = 'draw' THEN
    PERFORM public._settle_duel(d.id, 'draw');
    RETURN 'draw';
  ELSE
    UPDATE public.duels
      SET status = 'dispute',
          dispute_reason = 'Inconsistent votes: player1=' || v1 || ', player2=' || v2,
          manual_review_requested_at = now(),
          manual_review_due_at = now() + interval '24 hours'
      WHERE id = d.id;
    PERFORM public._notify(d.player1_id, 'duel_dispute', 'Dispute opened', 'Votes do not match.', '/duels/' || d.id);
    PERFORM public._notify(d.player2_id, 'duel_dispute', 'Dispute opened', 'Votes do not match.', '/duels/' || d.id);
    PERFORM public._notify_admins('dispute_pending', 'Dispute to review', 'A duel is in dispute.', '/admin');
    RETURN 'dispute';
  END IF;
END;
$$;
...
BEGIN
  SELECT * INTO d FROM public.duels WHERE id = p_duel FOR UPDATE;
  IF NOT FOUND OR v_me NOT IN (d.player1_id, d.player2_id) THEN
    RAISE EXCEPTION 'Access denied';
  END IF;
  IF d.status NOT IN ('active', 'waiting_votes') THEN
    RAISE EXCEPTION 'Duel cannot be disputed';
  END IF;
  UPDATE public.duels
    SET status = 'dispute',
        ...
        manual_review_due_at = now() + interval '24 hours'
    WHERE id = d.id;
  PERFORM public._notify_admins('dispute_pending', 'Dispute reported', 'A player opened a dispute.', '/admin');
END;
$$;
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE reference = TRIM(p_reference) AND status <> 'rejected'
  ) THEN
    RAISE EXCEPTION 'This transaction reference was already used';
  END IF;
  IF (SELECT COUNT(*) FROM public.deposits WHERE user_id = v_me AND status = 'pending') >= 3 THEN
    v_flags := v_flags || 'multiple_pending_deposits';
    v_score := v_score + 40;
  END IF;
  IF p_amount > 500000 THEN
    v_flags := v_flags || 'high_amount';
    v_score := v_score + 30;
  END IF;

  INSERT INTO public.deposits (
    user_id, amount, method, sender_name, sender_phone, reference, screenshot, fraud_score, fraud_flags
  )
  VALUES (
    v_me, p_amount, p_method, p_sender_name, p_sender_phone, TRIM(p_reference), p_screenshot, v_score, v_flags
  )
  RETURNING id INTO v_id;
  ...
  PERFORM public._notify_admins(
    'deposit_pending',
    'Deposit to review',
    'Deposit of ' || p_amount || ' FCFA is waiting for review.',
    '/admin'
  );
...
BEGIN
  IF v_me IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  IF p_amount < 1000 THEN RAISE EXCEPTION 'Minimum withdrawal is 1,000 FCFA'; END IF;
  ...
  IF (SELECT total_deposited FROM public.wallets WHERE user_id = v_me) = 0 THEN
    v_flags := v_flags || 'no_previous_deposit';
    v_score := v_score + 50;
  END IF;

  INSERT INTO public.withdrawals (
    user_id, amount, method, phone_number, net_amount, fraud_score, fraud_flags
  )
  VALUES (v_me, p_amount, p_method, p_phone, p_amount, v_score, v_flags)
  RETURNING id INTO v_id;
  ...
  PERFORM public._record_tx(v_me, 'withdrawal', p_amount, v_bal, v_bal - p_amount, 'Pending withdrawal', NULL, NULL, v_id);

  PERFORM public._notify_admins(
    'withdrawal_pending',
    'Withdrawal to review',
    'Withdrawal of ' || p_amount || ' FCFA is waiting for review.',
    '/admin'
  );
...
DECLARE
  ...
  v_me uuid := auth.uid();
BEGIN
  IF COALESCE(TRIM(p_new_username), '') = '' THEN
  ...
  VALUES (v_me, TRIM(p_new_username), p_reason)
  RETURNING id INTO v_id;

  PERFORM public._notify_admins(
    'username_change_pending',
    'Username change request',
    'A username change request needs review.',
    '/admin'
  );
...
AS $$
DECLARE
  dp public.deposits%ROWTYPE;
  v_bal numeric;
BEGIN
  PERFORM public._require_admin();
  SELECT * INTO dp FROM public.deposits WHERE id = p_deposit FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Deposit not found'; END IF;
  IF dp.status <> 'pending' THEN RAISE EXCEPTION 'Deposit already processed'; END IF;

  IF p_approve THEN
    SELECT balance_available INTO v_bal FROM public.wallets WHERE user_id = dp.user_id FOR UPDATE;
    UPDATE public.wallets
      SET balance_available = balance_available + dp.amount,
          total_deposited = total_deposited + dp.amount
      WHERE user_id = dp.user_id;
    PERFORM public._record_tx(dp.user_id, 'deposit', dp.amount, v_bal, v_bal + dp.amount, 'Approved deposit (' || dp.reference || ')', NULL, dp.id);
    UPDATE public.deposits
      SET status = 'approved',
          ...
          reviewed_at = now()
      WHERE id = dp.id;
    PERFORM public._notify(dp.user_id, 'deposit_approved', 'Deposit approved', dp.amount || ' FCFA credited.', '/portefeuille');
  ELSE
    UPDATE public.deposits
      SET status = 'rejected',
          admin_note = p_note,
          ...
          reviewed_at = now()
      WHERE id = dp.id;
    PERFORM public._notify(dp.user_id, 'deposit_rejected', 'Deposit rejected', COALESCE(p_note, 'Reference could not be verified.'), '/portefeuille');
  END IF;
...
BEGIN
  ...
  IF wd.status <> 'pending' THEN RAISE EXCEPTION 'Withdrawal already processed'; END IF;

  IF p_approve THEN
    UPDATE public.wallets
      SET balance_locked = balance_locked - wd.amount,
    ...
    UPDATE public.withdrawals
      SET status = 'approved',
          ...
          reviewed_at = now()
      WHERE id = wd.id;
    PERFORM public._notify(wd.user_id, 'withdrawal_approved', 'Withdrawal approved', wd.amount || ' FCFA sent to ' || wd.phone_number || '.', '/portefeuille');
  ELSE
    SELECT balance_available INTO v_bal FROM public.wallets WHERE user_id = wd.user_id FOR UPDATE;
    UPDATE public.wallets
      SET balance_locked = balance_locked - wd.amount,
          balance_available = balance_available + wd.amount
      WHERE user_id = wd.user_id;
    PERFORM public._record_tx(wd.user_id, 'stake_refunded', wd.amount, v_bal, v_bal + wd.amount, 'Withdrawal rejected: funds returned', NULL, NULL, wd.id);
    UPDATE public.withdrawals
      SET status = 'rejected',
          admin_note = p_note,
          reviewed_by = auth.uid(),
          reviewed_at = now()
      WHERE id = wd.id;
    PERFORM public._notify(wd.user_id, 'withdrawal_rejected', 'Withdrawal rejected', COALESCE(p_note, 'Request denied.'), '/portefeuille');
  END IF;
...
BEGIN
  ...
  UPDATE public.profiles
    SET is_banned = p_banned,
        status = CASE WHEN p_banned THEN 'banned'::public.user_status ELSE 'active'::public.user_status END
    WHERE id = p_user;
  PERFORM public._notify(
    p_user,
    'account_status',
    CASE WHEN p_banned THEN 'Account suspended' ELSE 'Account reactivated' END,
    COALESCE(p_note, ''),
    '/profil'
  );
...
BEGIN
  ...
  PERFORM public._record_tx(p_user, 'adjustment', p_amount, v_bal, v_bal + p_amount, COALESCE(p_note, 'Admin adjustment'));
  PERFORM public._notify(p_user, 'balance_adjusted', 'Balance adjusted', 'Adjustment of ' || p_amount || ' FCFA. ' || COALESCE(p_note, ''), '/portefeuille');
  PERFORM public._admin_log('balance_adjusted', 'User', p_user, p_note, jsonb_build_object('amount', p_amount));
END;
$$;

CREATE OR REPLACE FUNCTION public.admin_review_username_change(
  p_request uuid,
  p_approve boolean,
  p_note text DEFAULT NULL
)
...
AS $$
DECLARE
  r public.username_change_requests%ROWTYPE;
BEGIN
  PERFORM public._require_admin();
  ...
  IF p_approve THEN
    UPDATE public.profiles
      SET username = r.new_username,
          efootball_username = r.new_username
      WHERE id = r.user_id;
    PERFORM public._notify(r.user_id, 'username_approved', 'Username updated', 'Your username is now "' || r.new_username || '".', '/profil');
  ELSE
    PERFORM public._notify(r.user_id, 'username_rejected', 'Request rejected', COALESCE(p_note, 'Username change rejected.'), '/profil');
  END IF;
...
BEGIN
  ...
  IF v_text = '' AND p_attachment_path IS NULL THEN RAISE EXCEPTION 'Empty message'; END IF;
  ...
  IF NOT FOUND OR v_me NOT IN (c.user_low, c.user_high) THEN
    RAISE EXCEPTION 'Access denied';
  END IF;
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id = v_me AND is_banned) THEN
    RAISE EXCEPTION 'Banned account: sending disabled';
  END IF;
  ...
  UPDATE public.conversations
    SET last_message_at = now(),
        last_message_preview = CASE WHEN v_text <> '' THEN LEFT(v_text, 120) ELSE 'Attachment sent' END
    WHERE id = p_conversation;

  PERFORM public._notify(v_other, 'message_received', 'New message', COALESCE(LEFT(v_text, 120), 'A file was sent.'), '/messages?c=' || p_conversation);
  RETURN v_id;
END;
$$;
...
BEGIN
  ...
  LOOP
    IF d.player1_vote IS NOT NULL THEN
      v_vote := d.player1_vote;
      v_voter := d.player1_id;
      v_other := d.player2_id;
    ELSE
      v_vote := d.player2_vote;
      v_voter := d.player2_id;
      v_other := d.player1_id;
    END IF;

    IF v_vote = 'draw' THEN
      PERFORM public._settle_duel(d.id, 'draw', NULL, 'Auto-closed after 24h of silence.');
    ELSIF v_vote = 'win' THEN
      PERFORM public._settle_duel(d.id, 'winner', v_voter, 'Auto-closed after 24h of silence.');
    ELSE
      PERFORM public._settle_duel(d.id, 'winner', v_other, 'Auto-closed after 24h of silence.');
    END IF;

    PERFORM public._notify(
      v_voter,
      'duel_auto_closed',
      'Duel closed automatically',
      'Your opponent did not vote in time.',
      '/duels/' || d.id
    );
    PERFORM public._notify(
      v_other,
      'duel_auto_closed',
      'Duel closed automatically',
      'The duel was closed automatically.',
      '/duels/' || d.id
    );
    PERFORM public._admin_log('duel_auto_closed', 'Duel', d.id, 'Automatic close after 24h without vote', '{}'::jsonb);
  END LOOP;
  ...
  LOOP
    ...
    PERFORM public._notify(
      ...
      'No admin review arrived in time.',
      '/duels/' || d.id
    );
    PERFORM public._notify(
      d.player2_id,
    ...
    PERFORM public._admin_log('dispute_auto_closed', 'Duel', d.id, 'Automatic dispute close after timeout', '{}'::jsonb);
  END LOOP;
END;
$$;
...
BEGIN
  ...
  INSERT INTO public.profiles (
    id, username, efootball_username, first_name, last_name, country, level
  )
  VALUES (
    NEW.id,
    v_username,
